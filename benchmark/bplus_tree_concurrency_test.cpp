...
public:
  ...
  virtual void SetUp(const State &state)
  {
    ...
    std::vector<AttrType> attrTypes = {INTS};
    std::vector<int> len = {sizeof(int32_t)};
  
    RC rc = handler_.create(filename, attrTypes, len /*attr_len*/, internal_max_size, leaf_max_size);
    if (rc != RC::SUCCESS) {
